...
def run_generate(verbose=True):
    ...
    parser.add_argument("--num_beams", type=int, default=6, required=False, help="batch size")
    parser.add_argument(
    ...
    loss_seq = generate_summaries_or_translations(
        examples,
        references,
        args.save_path,
        args.model_name,
    ...
    with open(
        "/".join(args.reference_path.split("/")[:-1])+"/teacher_ppl.txt", "wb"
        ) as f:
        np.save(f, np.array(loss_seq))
    return scores
