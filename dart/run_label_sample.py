...
def generate_summaries_or_translations(
    ...
    **generate_kwargs,
    ...
    for i, example in enumerate(file_dict['entries']):
        ...
        #print(dec)
        dec = [hypothesis.lstrip().rstrip() for hypothesis in dec]
        #for jj, hypothesis in enumerate(dec):
        #    hypothesis = hypothesis.lstrip().rstrip()
            #fout.write(upper_first(hypothesis.lstrip())+'<|endoftext|>' + "\n")
        new_example = copy.deepcopy(example)
        new_example[str(i+1)]['lexicalisations'] = [{
                "comment": "good",
                "lang": "",
                "lex": hypothesis,
                "xml_id": "Id{0}".format(jj)
            } for jj, hypothesis in enumerate(dec)]
        new_file_dict['entries'].append(new_example)
        #print(dec[:3])    
        outputs += dec
...
def run_generate(verbose=True):
    ...
    parser.add_argument("--max_src_length", type=int, default=128, required=False)
    parser.add_argument("--num_samps", type=int, default=1, required=True)
    parser.add_argument("--top_k", type=int, default=30, required=True)
    parser.add_argument("--top_p", type=float, default=1., required=True)
    ...
    scores = calc_eval_metrics(output_lns, reference_lns)
    print(scores)
