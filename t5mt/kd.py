...
class SummarizationDistiller(TranslationModule):
    ...
    def __init__(self, hparams):
        ...
        #self.decoder_start_token_id = 250020#self.tokenizer.lang_code_to_id[hparams.tgt_lang]
        ...
        self.teacher_temperature = hparams.teacher_temperature
    ...
    def calc_ce_loss(self, mask, s_logits, t_logits, s_temp=None, t_temp=None, add_eps=False):
        """Copy pasted from distillbert (transformers/examples/distillation/)"""
        # mask has False at padding_idx
        s_temp = s_temp if s_temp is not None else self.temperature
        t_temp = t_temp if t_temp is not None else self.temperature
        ...
        assert not torch.isnan(t_logits_slct).any()
        assert not torch.isinf(t_logits_slct).any()
        assert not torch.isnan(s_logits_slct).any()
        assert not torch.isinf(s_logits_slct).any()
        global STUDENT_OUT
        global TEACHER_OUT

        loss_ce = (
            self.ce_loss_fct(
                F.log_softmax(s_logits_slct / s_temp, dim=-1), # bottom
                F.softmax(t_logits_slct / t_temp, dim=-1) + (1e-40 if add_eps else 0.), # up 
            )
            * 1.
        )
        assert not torch.isnan(loss_ce).any()
        return loss_ce

    def calc_js_loss(self, mask, s_logits, t_logits, s_wght=0.5, t_wght=0.5, add_eps=False):
        """Copy pasted from distillbert (transformers/examples/distillation/)"""
        # mask has False at padding_idx
        sel_mask = mask[:, :, None].expand_as(s_logits)
        vocab_size = s_logits.size(-1)
        ...
        loss_ce = (
            self.ce_loss_fct(
                ...
                F.softmax(s_logits_slct / self.temperature, dim=-1) + (1e-40 if add_eps else 0.) # up
            )
            * 1.
        )
        return loss_ce
    ...
    def calc_engine_loss(self, mask, s_logits, t_logits):
        ...
        #print((s_logits * t_logits)[0][:, self.tokenizer.eos_token_id].half().tolist())
        sel_mask = mask[:, :, None].expand_as(s_logits)
        vocab_size = s_logits.size(-1)
        s_logits_slct = torch.masked_select(s_logits, sel_mask)  # (bs * seq_length * voc_size) modulo the 1s in mask
        t_logits_slct = torch.masked_select(t_logits, sel_mask)  # (bs * seq_length * voc_size) modulo the 1s in mask
        #print((s_logits_slct * t_logits_slct)[0].half().tolist())
    ...
    def _engine_step(self, batch):
        ...
        decoder_input_ids = self.model._shift_right(student_pred_ids)
    ...
    def _kl_sample_step(self, batch: dict) -> torch.tensor:
        ...
        decoder_input_ids = self.model._shift_right(teacher_pred_ids)
    ...
    def _generate(self, batch: dict, gen_by_teacher=False):
        ...
        #print(batch["labels"].size(1), "labels")
    ...
    def _js_student_step(self, batch):
        ...
        dec_mask = student_pred_ids.ne(pad_token_id)
        loss_js = self.calc_js_loss(
            dec_mask, lm_logits, teacher_outputs["logits"], 
            s_wght=self.beta, t_wght=(1.-self.beta), add_eps=True)
    ...
    def _rkl_step(self, batch: dict) -> torch.tensor:
        ...
        lm_logits = student_outputs["logits"]
        assert not torch.isnan(lm_logits).any()
        with torch.no_grad():
            teacher_outputs = self.teacher(
                input_ids,
                attention_mask=src_mask,
        ...
        assert lm_logits.shape[-1] == self.model.config.vocab_size
        dec_mask = student_pred_ids.ne(pad_token_id)
        loss = self.calc_ce_loss(
            dec_mask, teacher_outputs["logits"], lm_logits, 
            s_temp=self.teacher_temperature, add_eps=True
        )
        return loss
