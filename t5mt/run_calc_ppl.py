...
def generate_summaries_or_translations(
    ...
    **generate_kwargs,
    ...
    for examples_chunk, references_chunk in tqdm(
        ...
        ce_loss_fct_2 = torch.nn.CrossEntropyLoss(ignore_index=tokenizer.pad_token_id)
        ...
        with torch.no_grad():
            ...
            all_batch_loss = ce_loss_fct(logits.view(-1, logits.shape[-1]), tgt_ids.view(-1)).view(bsz, -1).sum(0)
            _loss = ce_loss_fct(logits.view(-1, logits.shape[-1]), tgt_ids.view(-1)).view(bsz, -1)
            lens += tgt_ids.ne(tokenizer.pad_token_id).float().sum()
